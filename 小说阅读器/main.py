...
#LastEditTime : 2020-05-08 14:36:13
...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_3.setText(_translate("MainWindow", "（比如6_6453）"))
        self.pushButton.setText(_translate("MainWindow", "选择"))
        self.pushButton_2.setText(_translate("MainWindow", "确定"))
        self.tabWidget.setTabText(
            self.tabWidget.indexOf(self.tab), _translate("MainWindow", "列表显示"))
        self.tabWidget.setTabText(
            self.tabWidget.indexOf(self.tab_2),
            _translate("MainWindow", "图表显示"))
        # 添加代码（设置列表标题）
    ...
    def getData(self, url, path):
        ...
        for item in links[8:20]:  # 遍历文章列表
            ...
            # 提取章节内容
            article_content = re.findall(r'id="content">(.*?)</div>',
                                         articleHtml, re.S)[0]
            # 过滤掉内容的间隔符、换行符等
            ...
            title = re.findall(r'<h1>(.*?)</h1>', articleHtml,
                               re.S)[0]  # 获取文章标题
            ...
            newFile.close()  # 关闭文件
        QMessageBox.Information(None, "提示", self.book_number + "的小说保存完成",
                                QMessageBox.Ok)
    ...
    # 获取所有文件
    def getFiles(self):
        self.list = os.listdir(self.lineEdit_2.text() + '\\' +
                               self.lineEdit.text())  # 列出文件夹下所有的目录与文件
    ...
    def tableClick(self, item):
        if 'txt' in item.text():  # 点击文件名才弹出
            os.startfile(self.lineEdit_2.text() + '\\' + self.lineEdit.text() +
                         '\\' + item.text())
