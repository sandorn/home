...
def ReqSynthesizer(text, format='wav', audioFile='', method='post', callback=None):
    ...
    if 'audio/mpeg' == res.headers['Content-Type']:
        ...
        if audioFile == '':
            ...
            result['filename'] = f'{audioFile}_{SpeechReqMeta.voice}_{round(SpeechReqMeta.sample_rate/1000)}K.{format}'
...
class synthesizeClass:
    def __init__(self, text=None, appkey=Constant.appKey, token=Constant.token, audioFile='', format='wav', sample_rate=16000, voice='Aida', volume=100, speech_rate=0, pitch_rate=0, callback=None, method='post'):
        ...
        self.appkey = appkey
        self.token = token or get_token()[0]
        self.text = text
    ...
    def setparams(self, attr, value):
        ...
        if attr == 'text' and self.audioFile == '':
...
def Synthesizerprocess(text, audioFile='', callback=Synthesizer_MyCallback):
    client = NlsClient()
...
def TranscriberProcess(filepath, appkey=Constant.appKey, token=Constant.token):
    ...
    transcriber.set_token(token or get_token()[0])
...
def PostTransFile(audioFile, appkey=Constant.appKey, token=Constant.token, format='wav', sampleRate=16000, enablePunctuationPrediction=False, enableInverseTextNormalization=True, enableVoiceDetection=True):
    ...
    # 设置HTTPS Headers
    httpHeaders = {
        'X-NLS-Token': token or get_token()[0],
...
if __name__ == "__main__":
    ...
    ReqSynthesizer(longtext, audioFile=None, callback=play_callback)
'''
    audiofilepath = "D:/alibabacloud-nls-python-sdk/nls-sample-16k.wav"
    ...
    long_text_list = string_split_join_with_maxlen_list(longtext)
    from xt_Pygame import ReqSynthesizer_Thread_read

    ReqSynthesizer_Thread_read(long_text_list)
    ReqLongSynthesizer(longtext)

    # #短文字合成语音，限定300字符
    SYC = synthesizeClass( text )
    res = SYC.run()
    ...
    ReqSynthesizer(longtext, audioFile=None, callback=play_callback)
    filename = Synthesizerprocess(text )
    ...
    # #网络音频文件识别
    text, res = APITransUrl(urlLink)
    print('网络音频文件识别', text, res)

    # # 本地音频文件识别
    text, res  = PostTransFile( audiofilepath )  ##一句话识别，1分钟以内
    res = TranscriberProcess(filepath)
