...
async def test_reset_instance():
    ...
    print(
        f"重置后 - 是同一个实例吗? {MetaSingleton.has_instance()}, 当前值: {m3.value}"
    )
    ...
    print(
        f"重置后 - 是同一个实例吗? {MixinSingleton.has_instance()}, 当前值: {mx3.value}"
    )
    ...
    print(
        f"重置后 - 是同一个实例吗? {DecoratorSingleton.has_instance()}, 当前值: {d3.value}"
    )
...
async def test_reinit():
    ...
    print(
        f"重新初始化后 - 是同一个实例吗? {d4 is DecoratorSingleton.get_instance()}, 当前值: {d4.value}"
    )
