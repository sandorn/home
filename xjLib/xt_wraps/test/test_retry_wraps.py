...
# 重试后成功的函数测试
@retry_wraps(max_attempts=4, retry_exceptions=(ValueError,))
...
async def run_all_tests():
    ...
    print('\n3. 始终失败的函数测试:')
    res31 = always_fail_sync()
    print(f'3.1 同步函数预期失败 | {res31}')

    res32 = await always_fail_async()
    print(f'3.2 异步函数预期失败 | {res32}')
