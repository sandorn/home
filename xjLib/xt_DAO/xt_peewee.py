...
#LastEditTime : 2020-06-03 11:42:21
...
class engine:
    ...
    def _set_params(self):
        ...
        '''
        self.params = {attr: getattr(self.baseclass, attr) for attr in self.baseclass.__dict__ if not callable(getattr(self.baseclass, attr)) and not attr.startswith("__") and not attr == '_sa_class_manager'}
        ...
        if isinstance(result, (tuple, list)):
            for item in result:
                if isinstance(item, self.baseclass):
                    ...
                    res_list.append({key: getattr(item, key) for key in self.params.keys()})
    ...
    def f(self, conditions, show=False):
        result = self.session.query(self.baseclass).filter(self.baseclass.username.like(conditions)).all()
        if show:
            return self._result_refine(result)
        else:
            return result
...
if __name__ == '__main__':
    ...
    # 创建一个peewee数据库实例
    database = MySQLDatabase('bxflb', **{'charset': 'utf8', 'sql_mode': 'PIPES_AS_CONCAT', 'use_unicode': True, 'host': 'cdb-lfp74hz4.bj.tencentcdb.com', 'port': 10014, 'user': 'sandorn', 'password': '123456'})
    ...
    class Users2(BaseModel):
        ...
        会员到期日 = DateTimeField(constraints=[SQL("DEFAULT CURRENT_TIMESTAMP")], null=True)
    ...
    for i in range(10):
        Users2(username='王五' + str(i), password=50, 手机='1333333333' + str(i)).save()
'''
备注
# !根据已有数据库生成模型
