...
class PlayBase:
    def __init__fn(self, meta, textlist=None):
        meta.__init__(self)
        self.__dict__["__isQ"] = meta == QThread
        self.__dict__["_target"] = execute_tts
...
def create_read_thread(meta):
    ...
    return type(f"Synt_Read_{_name}", (meta,), {"_signal": pyqtSignal(), "__init__": PlayBase.__init__fn, "main_monitor": PlayBase.main_monitor, "run": PlayBase.run, "stop": PlayBase.stop})
