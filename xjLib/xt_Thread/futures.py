        ...
        while self.is_running or not self.task_queue.empty():
            try:
                ...
                if task is None:  # 退出信号
                    with self.lock:
                        # 安全地移除线程，先检查线程是否在列表中
                        if current_thread in self.workers:
                            self.workers.remove(current_thread)
                    break
    ...
    def get_pool(cls, max_workers: int | None = None) -> ThreadPoolExecutor:
        ...
        with cls._lock:
            ...
            if max_workers is None:
                target_max_workers = base_workers * 4  # 默认IO密集型配置
            else:
                target_max_workers = max_workers
...
class EnhancedThreadPool(Generic[_T, _R]):
    ...
    def set_exception_handler(self, handler: Callable[[Exception], None]) -> None:
        ...
        参数:
            handler: 异常处理函数，接收一个异常参数
        """
        self.exception_handler = handler
    
    def _task_wrapper(self, fn: Callable[[_T], _R], *args, **kwargs) -> _R:
        """任务包装器，用于收集结果和处理异常"""
        try:
    ...
    def submit_task(self, fn: Callable[[_T],_R], *args: Any, **kwargs: Any) -> Future:
        """提交异步任务到线程池
...
class TaskExecutor(ThreadPoolExecutor):
    ...
    def __init__(self, io_bound: bool = True, max_workers: int = None):
        """
        参数:
            io_bound: 是否为I/O密集型任务，True则设置较多线程
            max_workers: 可选，手动指定最大工作线程数，优先级高于io_bound
    ...
    def add_tasks(
        self,
        fn: Callable[..., _T],
        iterables: List[Tuple[Tuple, Dict]],
        callback: Callable[..., _T] = None,
    ):
        """并行执行函数，每个元素作为独立任务提交
        ...
        参数:
            fn: 要执行的目标函数
            *iterables: 可迭代对象，每个元素作为fn的参数
            timeout: 可选，超时时间（秒）
        """
        for arg, kwargs in iterables:
            self.add_task(fn, *arg, callback=callback, **kwargs)
...
class AsyncFunction:
    ...
    def __init__(self, fn, *args, max_workers: int = None, **kwargs):
