...
class AsyncProduction:
    ...
    async def start(
        ...
        # 创建任务并保存引用
        self.producer_tasks = [
            asyncio.create_task(self._producer_loop(producer_fn)) for _ in range(num_producers)
        ]
        self.consumer_tasks = [
            asyncio.create_task(self._consumer_loop(consumer_fn)) for _ in range(num_consumers)
        ]
...
if __name__ == '__main__':
    ...
    def run_sync_test():
        ...
        # 运行一段时间后停止
        mylog.debug(f"系统启动,当前任务数: {production_system.get_task_count()}")
        time.sleep(2)  # 让生产和消费运行2秒

        # 检查任务状态
        mylog.debug(f"运行中,当前任务数: {production_system.get_task_count()}")
        current_results = production_system.get_result_list()
        mylog.debug(f"当前已完成的结果数: {len(current_results)}")
        if current_results:
            mylog.debug(f"当前已完成的部分结果: {current_results[:3]}")

        # 测试单独获取结果
        if not production_system.res_queue.empty():
            single_result = production_system.get_result()
            mylog.debug(f"单独获取的一个结果: {single_result}")

        # 停止系统并获取结果
        results = production_system.shutdown()
        mylog.debug(f"测试完成,共处理了{len(results)}个任务")
        mylog.debug(f"前5个结果: {results[:5] if len(results) >= 5 else results}")
    ...
    async def run_async_test():
        ...
        async def error_handler(e, item):
            ...
            mylog.debug(f"异步处理{item}时发生错误: {e}")
        ...
        # 测试重复启动保护
        await system.start(
            num_producers=2, num_consumers=3, producer_fn=mock_producer, consumer_fn=mock_consumer
        )

        mylog.debug("异步系统启动成功")
        # 测试再次启动（应该不会有问题,因为有重复启动保护）
        await system.start(3, 4, mock_producer, mock_consumer)
        mylog.debug("验证重复启动保护正常工作")
