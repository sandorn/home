...
@LastEditTime: 2020-04-03 19:38:38
...
from xjLib.mystr import md5, random_20char, toMysqlDateTime
...
def login():
    ...
    response = Session.post('http://oa.jklife.com/seeyon/main.do?method=login', data=payload, allow_redirects=True)
    if response.status_code == 200:
        return Session


def get_download_url_save(Session, stop=None):
    connect = SqlHelper('Jkdoc')
    db_set = set()
    ...
    for index in range(1, pages + 1):
        _response = _run(Session, index)
        _dic = _response.json()
        _itlist = _dic['list']  # 当前页面项目列表
        for item in _itlist:
            if md5(item['id']) not in db_set:
                _urls.append((item['title'].strip(), item['id']))
                db_set.add(md5(item['id']))
            else:
                continue
...
def down_content(Session, connect, title, url):
    ...
    # #写txt文件
    公告正文 = ''.join(_res.html.xpath('//div[@class="contentText"]//text()'))
    with open(path + '/' + title + '.txt', 'w', encoding='utf-8') as f:
        f.write(title + '\n\n' + 公告正文)

    with mutexA:
        connect.insert({'TITLE': title, 'URL': url, 'TIME': toMysqlDateTime()}, 'jkdoc')

    # #下载附件
    _附件下载 = _res.html.xpath('//div[@id="attFileDomain"]')[0].get('attsdata')  # !关键get字符串str
...
def main():
    ...
    print('需要下载的公文数量为：', len(urls))
    # 创建多进程队列
    pool = Pool(25)
    _ = [pool.submit(down_content, Session, connect, item[0], item[1])for item in urls]
