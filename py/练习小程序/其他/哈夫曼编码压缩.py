...
@Software :   VSCode
@File     :   哈夫曼编码压缩.py
@Time     :   2019/05/06 10:05:19
@Author   :   Even Sand
@Version  :   1.0
@Contact  :   sandorn@163.com
@License  :   (C)Copyright 2009-2019, NewSea
@explain  :
Blog/Huffman.py at master · str-mo/Blog
https://github.com/str-mo/Blog/blob/master/Huffman/Huffman.py
基于哈夫曼编码的压缩算法的Python实现 - 字节莫的CSDN博客 - CSDN博客
...
def encodefile(inputfile):
    ...
    head = sorted(
        count_dict.items(), key=lambda x: x[1], reverse=True)  # 对所有根节点进行排序
    bit_width = 1
...
def decodefile(inputfile):
    ...
    f.seek(0)
    name = inputfile.split('/')
    outputfile = inputfile.replace(name[len(name) - 1],
                                   f.readline().decode(encoding="utf-8"))
...
if __name__ == '__main__':
    ...
    def fd():
        ...
        _dlg.SetOFNInitialDir('d:/')  # 设置打开文件对话框中的初始显示目录
        _dlg.DoModal()
