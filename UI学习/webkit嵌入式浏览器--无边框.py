...
'''
@Software:   VSCode
@File    :   webkit嵌入式浏览器--无边框.py
@Time    :   2019/05/05 11:10:46
@Author  :   Even Sand
@Version :   1.0
@Contact :   sandorn@163.com
@License :   (C)Copyright 2019-2019, NewSea
@Desc    :   None
PyQt5无边框窗口的标题拖动和窗口缩放实现 - qq_38528972的博客 - CSDN博客
...
class QUnFrameWindow(QWidget):
    ...
    def resizeEvent(self, QResizeEvent):
        ...
        self._right_rect = [
            QPoint(x, y)
            for x in range(self.width() - self._padding,
                           self.width() + 1)
            for y in range(1,
...
if __name__ == "__main__":
    ...
    _url = os.path.join(os.path.dirname(__file__), 'UnFrameStyle.qss')
    app.setStyleSheet(open(_url).read())
