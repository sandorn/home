...
@LastEditTime: 2019-05-27 19:04:50
'''
import sys
import time
...
class MyWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.texts = []  # 用于存放结果
        self.step = 0
        self._step.connect(self.upstep)
    ...
    def on_runObject_triggered(self):
        ...
        with ThreadPoolExecutor(20) as p:
            future_tasks = [p.submit(self.getkeys, url, _max) for url in self.urls]

        wait(future_tasks, return_when=ALL_COMPLETED)  # 等待到所有线程结束
        qApp.processEvents()  # 刷新显示
    ...
    # ! 批量创建子线程，子线程影讯结束后传递返回结果信号到主线程
    # ! 主线程wait 所有子线程结束，根据返回信号操作主线程UI界面
    # ! 主线程UI界面--> resultTable 不实时更新

    def getkeys(self, target, _max):
        (key, page, url) = target
        _res = []
