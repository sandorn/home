...
#Date         : 2020-05-21 17:12:13
#LastEditTime : 2020-05-21 17:18:57
#Github       : https://github.com/sandorn/home
#License      : (C)Copyright 2009-2020, NewSea
#==============================================================
https://muyuuuu.github.io/2019/10/06/pyqt5-basic/

A-Beautiful-and-Applicable-PyQt5-Interface/main.py at master · muyuuuu/A-Beautiful-and-Applicable-PyQt5-Interface
https://github.com/muyuuuu/A-Beautiful-and-Applicable-PyQt5-Interface/blob/master/main.py
'''
...
from PyQt5.QtWidgets import (
    QApplication, QSplitter, QGridLayout, QHBoxLayout, QPushButton, QTreeWidget,
    QFrame, QLabel, QHBoxLayout, QMainWindow, QStackedLayout, QWidget,
    QVBoxLayout, QLineEdit, QRadioButton, QTreeWidgetItem, QDesktopWidget,
    QTabWidget, QSpinBox, QComboBox, QCalendarWidget, QDateTimeEdit,
    QMessageBox, QAbstractItemView, QHeaderView, QTableWidget, QTableWidgetItem,
    QStyledItemDelegate, QSizePolicy, QTextEdit, QScrollArea, QGroupBox)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            QMessageBox.critical(None, ("无法打开数据库"), ("SQlite支持"),
                                 QMessageBox.Cancel)
            return False
        """
        此处完成布局页面的基本功能
        ...
        colleage_major_info_button_layout.addWidget(
            self.colleage_major_info_college)
        colleage_major_info_button_layout.addWidget(
            self.colleage_major_info_major)
        colleage_major_info_button_layout.addWidget(
            self.colleage_major_input_btn)
        ...
        colleage_major_info_layout.addWidget(
            self.table['colleage_major_twoside'])
        ...
        self.friend_tree.setHeaderLabels(
            ['年级', '人员, 网址, 个签, 曾获最高奖，点击打开网页（请用Chrome或Firefox打开）'])
        ...
        friend_label1 = QLabel(
            "注意：不支持加入在CSDN，博客园，知乎等借助第三方平台编写的无技术门槛的博客，鼓励与推荐hexo或jekyll。")
        ...
        self.input_contest_time_begincomo.setMinimumDate(
            QDate.currentDate().addDays(-1095))
        ...
        self.input_contest_time_endcomo.setMinimumDate(
            QDate.currentDate().addDays(-1095))
        ...
        self.input_contest_time_incomo.setMinimumDate(
            QDate.currentDate().addDays(-1095))
        ...
        for i in range(1, 4):
            ...
            self.team['team_layout' + str(i)].addWidget(self.team['team_id' +
                                                                  str(i)])
            self.team['team_layout' + str(i)].addWidget(self.team['team_name' +
                                                                  str(i)])
            self.team['team_layout' + str(i)].addWidget(self.team['team_price' +
                                                                  str(i)])
            self.team['team_layout' + str(i)].addWidget(
                self.team['team_college' + str(i)])
            self.team['team_layout' + str(i)].addWidget(self.team['team_major' +
                                                                  str(i)])
            self.team['team_layout' + str(i)].addWidget(
                self.team['team_gender' + str(i)])
        ...
        self.colleage_major_info_college.textChanged.connect(
            self.enable_colleage_major_input_btn)
        self.colleage_major_info_major.textChanged.connect(
            self.enable_colleage_major_input_btn)
        ...
        self.team["team_college1"].currentIndexChanged.connect(
            self.change_major1)
        self.team["team_college2"].currentIndexChanged.connect(
            self.change_major2)
        self.team["team_college3"].currentIndexChanged.connect(
            self.change_major3)

        # 比赛改变时 奖项跟随改变
        self.input_contest_class.currentIndexChanged.connect(
            self.select_contest_class)

        # 表格双击触发编辑事件，判断是否为空。
        # 空：表示在空白单元格输入信息 此是不允许
        # 非空：表示单元格内有内容 此时可以编辑
        self.table['input_contest_info_table'].cellDoubleClicked.connect(
            self.confirm_update)
        ...
        # 按教师查询 教师下拉框改变后触发
        self.output_teacher_comobo.currentTextChanged.connect(
            self.query_teacher)
    ...
    def select_contest_class(self):
        ...
        if self.input_contest_class.currentText() == '美国大学生数学建模竞赛':
            for i in range(1, 4):
                ...
                self.team['team_price' +
                          str(i)].addItem('Successful Participant')
                self.team['team_price' +
                          str(i)].addItem('Unsuccessful Participant')
    ...
    def init_table(self, string=''):
        ...
        if string in [
                'input_friend_table', 'contest_info_table', 'teacher_table',
                'registor_table'
        ]:
        ...
        # 视图整行选中的
        if string in [
                'input_friend_table', 'input_contest_info_table',
                'contest_info_table', 'teacher_table'
        ]:
            self.table[string].setSelectionBehavior(
                QAbstractItemView.SelectRows)

        # 自适应伸缩的 不能用elif
        if string in [
                'input_friend_table', 'contest_info_table',
                'colleage_major_twoside', 'teacher_table', 'registor_table'
        ]:
            self.table[string].horizontalHeader().setSectionResizeMode(
                QHeaderView.Stretch)
        ...
        if string == 'input_friend_table':
            ...
            self.table[string].setRowCount(1000)
            self.table[string].setHorizontalHeaderLabels([
                '学号', '姓名', ' 学院', ' 专业', '性别', '开始时间', '结束时间', '获奖时间', '指导教师',
                '比赛', '奖项级别'
            ])
    ...
    def query_teacher(self):
        ...
        self.query.execute(
            "select * from sducontest where teacher = \"{}\"".format(info_))
    ...
    def query_sdu(self):
        ...
        if (str.isdigit(info_)):
            self.query.execute(
                "select * from sducontest where id = \"{}\"".format(info_))
            results = self.query.fetchall()
        elif (str.isalpha(info_)):
            self.query.execute(
                "select * from sducontest where name = \"{}\"".format(info_))
            results = self.query.fetchall()
        else:
            QMessageBox.information(None, ("友情提示"), ("输入无效"),
                                    QMessageBox.Cancel)
            flag = 1
        if flag == 0:
            string = ""
    ...
    def confirm_update(self):
        ...
        try:
            ...
            self.table['input_contest_info_table'].setEditTriggers(
                QAbstractItemView.DoubleClicked)
        except:
            QMessageBox.information(None, ("友情提示"), ("空内容无法修改，只能修改已有内容"),
                                    QMessageBox.Cancel)
            self.table['input_contest_info_table'].setEditTriggers(
    ...
    def input_contest_one(self):
        ...
        for i in range(1, 4):
            ...
            begin_time.append(self.input_contest_time_begincomo.date().toString(
                Qt.ISODate))
            end_time.append(self.input_contest_time_endcomo.date().toString(
                Qt.ISODate))
            in_time.append(self.input_contest_time_incomo.date().toString(
                Qt.ISODate))
        ...
        for i in range(3):
            ...
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 0, QTableWidgetItem(id_[i]))
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 1, QTableWidgetItem(name_[i]))
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 2, QTableWidgetItem(college_[i]))
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 3, QTableWidgetItem(major_[i]))
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 4, QTableWidgetItem(gender_[i]))
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 5,
                QTableWidgetItem(begin_time[i]))
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 6, QTableWidgetItem(end_time[i]))
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 7, QTableWidgetItem(in_time[i]))
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 8, QTableWidgetItem(teacher_[i]))
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 9, QTableWidgetItem(contest_[i]))
            self.table['input_contest_info_table'].setItem(
                self.input_contest_row_count, 10, QTableWidgetItem(class_[i]))
    ...
    def change_major(self):
        ...
        self.query.execute(
            "select major_name from major where college_name = \"{}\"".format(
                college_))
    ...
    def del_college_major(self):
        ...
        try:
            ...
            if pos == 0:
                self.query.execute(
                    "delete from college where name = \"{}\"".format(info))
                self.db.commit()
                self.show_college(self.table['colleage_major_twoside'])
            if pos == 1:
                self.query.execute(
                    "delete from major where major_name = \"{}\"".format(info))
                self.db.commit()
                self.show_major()
        # 清空
        except:
            QMessageBox.information(None, ("友情提示"), ("请不要选择空行"),
                                    QMessageBox.Cancel)
    ...
    def insert_college_major(self):
        ...
        if major_name != "":
            self.query.execute(
                "insert into major values(\"{}\", \"{}\")".format(
                    major_name, college_name))
        elif major_name == "":
            QMessageBox.information(None, ("友情提示"), ("输入专业前请选择学院"),
                                    QMessageBox.Cancel)
        self.db.commit()
        self.show_college(self.table['colleage_major_twoside'])
        self.show_major()
    ...
    def show_major(self):
        ...
        try:
            ...
            self.query.execute(
                "select major_name from major where college_name = \"{}\""
                .format(college_info))
            rows = self.query.fetchall()
            for row in rows:
                inx = rows.index(row)
        ...
        # 空行对应的学院无法显示专业
            ...
            QMessageBox.information(None, ("友情提示"), ("请不要选择空行，无法显示专业"),
                                    QMessageBox.Cancel)
    ...
    def show_college(self, a):
        ...
        if isinstance(a, QTableWidget):
            for row in rows:
                ...
                self.table['colleage_major_twoside'].setItem(
                    inx, 0, college_name)
    ...
    def del_friend_table(self):
        ...
        """
        row_num = -1
        for i in self.table['input_friend_table'].selectionModel().selection(
        ).indexes():
            row_num = i.row()
    ...
    def process_contest(self):
        ...
        if text == '输入比赛':
            ...
            self.query.execute(
                "INSERT INTO contest VALUES(\"{}\", \"{}\", \"{}\")".format(
                    sponsor, class_, id_))
            self.db.commit()
            self.show_contest()
        if text == '删除比赛':
            row_num = -1
            for i in self.table['contest_info_table'].selectionModel(
            ).selection().indexes():
                row_num = i.row()
            if self.table['contest_info_table'].item(row_num, 0):
                del_id = self.table['contest_info_table'].item(row_num,
                                                               2).text()
                # 在数据库中删除
                self.query.execute("delete from contest where id = '%s'" %
                                   del_id)
                self.db.commit()
                QMessageBox.information(None, ("友情提示"), ("删除完毕"),
                                        QMessageBox.Ok)
                self.show_contest()
    ...
    def delete_teacher_info(self):
        ...
        """
        row_num = -1
        for i in self.table['teacher_table'].selectionModel().selection(
        ).indexes():
            row_num = i.row()
    ...
    def input_teacher_info(self):
        ...
        self.query.execute(
            "INSERT INTO teacher VALUES(\"{}\", \"{}\", \"{}\")".format(
                name, college, id))
        self.db.commit()
        self.show_teacher()
        # 关联查询界面的教师信息
    ...
    def del_admin(self):
        ...
        """
        row_num = -1
        for i in self.table['registor_table'].selectionModel().selection(
        ).indexes():
            row_num = i.row()
        if self.table['registor_table'].item(row_num, 0):
            ...
            if del_id != "wuyuhang@admin":
                ...
                QMessageBox.information(None, ("友情提示"), ("此帐号不能删除"),
                                        QMessageBox.Ok)
    ...
    def insert_admin(self):
        ...
        ls = []
        for row in self.query.execute("select id from admin where id = '%s'" %
                                      in_id):
            ls.append(row)
        if len(ls) != 0:
            QMessageBox.information(None, ("友情提示"), ("此账号已经注册过，请重新注册"),
                                    QMessageBox.Cancel)
        elif psd != confirm_psd:
            QMessageBox.information(None, ("友情提示"), ("两次输入的密码不一致"),
                                    QMessageBox.Cancel)
        else:
            # 加入密码不为空的逻辑判断
            if psd == confirm_psd and in_id != "" and psd != "":
                ...
                if self.query.execute(
                        "INSERT INTO admin VALUES(\"{}\", \"{}\")".format(
                            in_id,
                            hashlib.md5(psd.encode('utf-8')).hexdigest())):
                    QMessageBox.information(None, ("友情提示"), ("注册成功"),
                                            QMessageBox.Ok)
                    self.db.commit()
                else:
                    QMessageBox.information(None, ("友情提示"), ("注册失败"),
                                            QMessageBox.Cancel)
        self.show_admin()
    ...
    def confirm_password(self):
        ...
        in_psd = hashlib.md5(
            self.password_line.text().encode('utf-8')).hexdigest()
        # 超级管理员
        ...
        if info:
            ...
            if self.user_line.text() == 'wuyuhang@admin':
                if in_psd == info[0][1]:
                    ...
                    QMessageBox.information(None, ("友情提示"), ("密码输入错误"),
                                            QMessageBox.Cancel)
            # 普通管理员
            else:
                if in_psd == info[0][1]:
    ...
    def show_friend_page(self):
        ...
        if self.page_count == 2:
            ...
            for j in rows:
                ls.append(str(j[0]))
                locals()['child_' + str(j[0])] = QTreeWidgetItem(
                    self.friend_tree)
            ...
            string = ""
            for row in rows:
                locals()['child_' + str(row[2]) + '_' +
                         str(key)] = QTreeWidgetItem(locals()['child_' +
                                                              str(row[2])])
                for j in row:
                    # 不同项之间加入空格 美化显示
                    if str(j) not in ls:
                        string += (str(j) + '  ')
                locals()['child_' + str(row[2]) + '_' + str(key)].setText(
                    1, string)
                key += 1
                string = ""
    ...
    def quit_act(self):
        ...
        message = QMessageBox.question(self, ' ', '确认退出吗？',
