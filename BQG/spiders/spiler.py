...
@LastEditTime: 2020-02-21 15:04:19
'''
...
class Spider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if _BOOKNAME not in self.db:
            ...
            # #构建redis字典，用于去重
            self.res_db[_BOOKNAME] = redis.Redis(host='62.234.220.66', port=6379, db=4)  # '62.234.220.66'
            ...
            for _ZJNAME in pandasData['ZJNAME']:
                self.res_db[_BOOKNAME].hset(self.res_dict[_BOOKNAME], _ZJNAME, 0)
        ...
        for index in range(len(全部章节节点)):
            ...
            if not self.res_db[_BOOKNAME].hexists(self.res_dict[_BOOKNAME], _ZJNAME):
                ...
                request = scrapy.Request(real_url, meta={'index': index}, callback=self.parse_content)
                yield request
            else:
    ...
    def parse_content(self, response):
        ...
        item['ZJNAME'] = response.xpath('//h1/text()').extract_first().replace('\xa0', ' ')
