...
class PipelineSqlCheck(object):
    def __init__(self):
        ...
        if conf.get('type'):
            conf.pop('type')
        self.connect = MySQLdb.connect(**conf)
    ...
    def process_item(self, item, spider):
        ...
        if _BOOKNAME not in self.db:
            ...
            self.redis_db[_BOOKNAME] = redis.Redis(host='127.0.0.1',
                                                   port=6379,
                                                   db=4)
            ...
            for res in pandasData['ZJNAME']:
                self.redis_db[_BOOKNAME].hset(self.redis_dict[_BOOKNAME], res,
                                              0)
            self.redb.add(_BOOKNAME)

        _ZJNAME = item['ZJNAME']
        if self.redis_db[_BOOKNAME].hexists(self.redis_dict[_BOOKNAME],
                                            _ZJNAME):
            ...
            self.redis_db[_BOOKNAME].hset(self.redis_dict[_BOOKNAME], _ZJNAME,
                                          0)
            return item
...
class PipelineToSqlTwisted(object):
    ...
    def from_settings(cls, settings):
        ...
        if conf.get('type'):
            conf.pop('type')
        dbpool = adbapi.ConnectionPool("MySQLdb", **conf)
        return cls(dbpool)
