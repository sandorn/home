...
def get_download_url(connect, stop=None):
    # #登录,并保存cookies
    res = Session["post"]("https://oa.jklife.com/seeyon/main.do?method=login", allow_redirects=False, data={"login_username": "liuxinjun", "login_password": "U2FsdGVkX1+FSC7gcXU2/WpBDDXBWtbz5xfyK3PmX1w="})
    print(res, res.headers, "||||||||||||||||||", Session.sson.cookies)
    data = {"method": "bulIndex", "typeId": 1, "CSRFTOKEN": "RBOJ-GFZV-HT0M-PHSV-RFSB-950L-SSED-W3L6"}
    ...
    # #获取公文列表
    getlistdata = {"managerMethod": "findBulDatas", "method": "ajaxAction", "managerName": "bulDataManager", "rnd": 87434, "typeId": 1, "CSRFTOKEN": "RBOJ-GFZV-HT0M-PHSV-RFSB-950L-SSED-W3L6", "arguments": json.dumps([{"pageSize": "9999", "pageNo": 1}])}
    # #"typeId": 区分类别,1为公告
...
def down_content(connect, title, url):
    ...
    if _附件下载:
        for item in json.loads(_附件下载):  # 转dict
            formdata = {"method": "download", "fileId": item["fileUrl"], "v": item["v"], "createDate": item["createdate"].split(" ")[0], "filename": item["filename"]}
